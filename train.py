...
def set_train_args():
    ...
    parser.add_argument("--output_path", type=str, default='output/bert_softmax_huggingface', help='模型与预处理数据的存放位置')
...
def main(args):
    ...
    if args.load_model_weights:
        model = BertSoftmaxForNer_.from_pretrained(args.pretrain_model_path, config=config)
    else:
        model = BertSoftmaxForNer_(config=config)
